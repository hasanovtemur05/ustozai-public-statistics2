...
export const useUserCertificateList = (currentPage: number, courseId?: string , region?:string,district?:string,date?:DateRange) => {
  const initialData = {
    data: getDatasList(),
    pagenationInfo: {
      total_pages: 0,
  ...
  const { data = initialData, ...args } = useQuery({
    queryKey: ['user_certificate_list', currentPage , courseId,region,district, date],
